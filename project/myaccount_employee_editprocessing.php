...
if(validateEmail($_POST['email'])){
    header("Location: myaccount_employee_edit.php?errorNumber=1");
    exit();
}

//SPRAWDZENIE, CZY IMIĘ JEST PRAWIDŁOWE
if(validateName($_POST['name'])){
    header("Location: myaccount_employee_edit.php?errorNumber=8");
    exit();
}
...
if(validateDateFormat($_POST['birthday'])){
    header("Location: myaccount_employee_edit.php?errorNumber=9");
    exit();
}

//SPRAWDZENIE, CZY PESEL MA POPRAWNY FORMAT
if(validatePESEL($_POST['pesel'])){
    header("Location: myaccount_employee_edit.php?errorNumber=10");
    exit();
}

//SPRAWDZENIE, CZY NAZWA MIEJSCOWŚCI JEST POPRAWNA
if(validateAddress($_POST['city'])){
    header("Location: myaccount_employee_edit.php?errorNumber=11");
    exit();
}

//SPRAWDZENIE, CZY KOD POCZTOWY JEST POPRAWNY
if(validateZipCodePl($_POST['code'])){
   header("Location: myaccount_employee_edit.php?errorNumber=12");
    exit(); 
}
...
if(validateHouseNr($_POST['house_nr'])){
    header("Location: myaccount_employee_edit.php?errorNumber=13");
    exit();
}

//SPRAWDZENIE, CZY NUMER MIESZKANIA JEST POPRAWNY, MOŻE BYĆ PUSTY
if(validateApartmentNr($_POST['apartment_nr'])){
    header("Location: myaccount_employee_edit.php?errorNumber=14");
    exit();
}
...
if($stmt = $mysqli->prepare($update_logowanie)){
    ...
    header("Location: myaccount_employee_edit.php?errorNumber=2");
    exit();
}
        ...
        if($stmt = $mysqli->prepare($query_adresy)){
            ...
            if (empty($ID_ADRES_FROM_DATABASE)){
                ...
                if($stmt = $mysqli->prepare($update_adresy)){
                    ...
                    header("Location: myaccount_employee_edit.php?errorNumber=3");
                    exit();
                }
            } 
            ...
            header("Location: myaccount_employee_edit.php?errorNumber=5");
            exit();
        }
        ...
        if($stmt = $mysqli->prepare($query_adresy)){
            ...
            if (empty($ID_ADRES)){
                ...
                if($stmt = $mysqli->prepare($insert_adresy)){
                    ...
                    header("Location: myaccount_employee_edit.php?errorNumber=4");
                    exit();
                }
...
if((isset($ID_ADRES_FROM_DATABASE)) && (!empty($ID_ADRES_FROM_DATABASE))){
    ...
    if($stmt = $mysqli->prepare($update_pracownicy)){
        ...
        header("Location: myaccount_employee_edit.php?errorNumber=6");
        exit();
    }
    ...
    if($stmt = $mysqli->prepare($delete_adresy)){
        ...
        header("Location: myaccount_employee_edit.php?errorNumber=7");
        exit();
    }
...
header("Location: myaccount_employee.php?infoNumber=1");
exit();
