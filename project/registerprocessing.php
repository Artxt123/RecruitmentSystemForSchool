...
if(validatePassword($_POST['password'],$_POST['password2'])){
    ...
    header("Location: register.php?errorNumber=" . $errorNumber);
    exit();
}
...
if($stmt = $mysqli->prepare("SELECT * FROM logowanie WHERE EMAIL=?")){
    ...
    if ($result->num_rows == 1){
        header("Location: register.php?errorNumber=3");
        exit();
    }
    $stmt->close();
}
...
if($stmt = $mysqli->prepare($query_adresy)){
    ...
    if (empty($ID_ADRES)){
        ...
        if($stmt = $mysqli->prepare($insert_adresy)){
            ...
            header("Location: register.php?errorNumber=5");
            exit();
        }
        ...
        if($stmt = $mysqli->prepare($query_adresy)){
            ...
            header("Location: register.php?errorNumber=6");
            exit();
        }
    } 
...
if($stmt = $mysqli->prepare("INSERT INTO logowanie (EMAIL, HASLO) VALUES (?,?)")){
    ...
    header("Location: register.php?errorNumber=4");
    exit();
}

//WPROWADZENIE DANYCH DO TABELI kandydaci
$insert_kandydaci = "INSERT INTO kandydaci (IMIE, NAZWISKO, PLEC, DATA_URODZENIA, PESEL, EMAIL, ID_ADRES) VALUES (?,?,?,?,?,?,?)";

if ($stmt = $mysqli->prepare($insert_kandydaci)){
    $stmt->bind_param("ssssssi",$name,$surname,$gender,$birthday,$pesel,$email,$ID_ADRES);
    $stmt->execute();
    $stmt->close();
} else {
    header("Location: register.php?errorNumber=7");
    exit();
}
