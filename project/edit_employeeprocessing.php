...
if(validateEmail($_POST['email'])){
    header("Location: edit_employee.php?errorNumber=1&email=" . $_POST['old_email']);
    exit();
}

//SPRAWDZENIE, CZY IMIĘ JEST PRAWIDŁOWE
if(validateName($_POST['name'])){
    header("Location: edit_employee.php?errorNumber=8&email=" . $_POST['old_email']);
    exit();
}
...
if(validateDateFormat($_POST['birthday'])){
    header("Location: edit_employee.php?errorNumber=9&email=" . $_POST['old_email']);
    exit();
}

//SPRAWDZENIE, CZY PESEL MA POPRAWNY FORMAT
if(validatePESEL($_POST['pesel'])){
    header("Location: edit_employee.php?errorNumber=10&email=" . $_POST['old_email']);
    exit();
}

//SPRAWDZENIE, CZY NAZWA MIEJSCOWŚCI JEST POPRAWNA
if(validateAddress($_POST['city'])){
    header("Location: edit_employee.php?errorNumber=11&email=" . $_POST['old_email']);
    exit();
}

//SPRAWDZENIE, CZY KOD POCZTOWY JEST POPRAWNY
if(validateZipCodePl($_POST['code'])){
   header("Location: edit_employee.php?errorNumber=12&email=" . $_POST['old_email']);
    exit(); 
}
...
if(validateHouseNr($_POST['house_nr'])){
    header("Location: edit_employee.php?errorNumber=13&email=" . $_POST['old_email']);
    exit();
}

//SPRAWDZENIE, CZY NUMER MIESZKANIA JEST POPRAWNY, MOŻE BYĆ PUSTY
if(validateApartmentNr($_POST['apartment_nr'])){
    header("Location: edit_employee.php?errorNumber=14&email=" . $_POST['old_email']);
    exit();
}
...
//DANE KANDYDATA
$ID_PRAC = filter_input(INPUT_POST,'ID_PRAC', FILTER_SANITIZE_NUMBER_INT);
...
if($stmt = $mysqli->prepare($update_logowanie)){
    ...
    header("Location: edit_employee.php?errorNumber=2&email=" . $_POST['old_email']);
    exit();
}
        ...
        if($stmt = $mysqli->prepare($query_adresy)){
            ...
            if (empty($ID_ADRES_FROM_DATABASE)){
                ...
                if($stmt = $mysqli->prepare($update_adresy)){
                    ...
                    header("Location: edit_employee.php?errorNumber=3&email=" . $_POST['old_email']);
                    exit();
                }
            } 
            ...
            header("Location: edit_employee.php?errorNumber=5&email=" . $_POST['old_email']);
            exit();
        }
        ...
        if($stmt = $mysqli->prepare($query_adresy)){
            ...
            if (empty($ID_ADRES)){
                ...
                if($stmt = $mysqli->prepare($insert_adresy)){
                    ...
                    header("Location: edit_employee.php?errorNumber=4&email=" . $_POST['old_email']);
                    exit();
                }
...
$update_pracownicy = "UPDATE pracownicy SET IMIE = ?, NAZWISKO = ?, PLEC = ?, DATA_URODZENIA = ?, PESEL = ?, ID_ADRES = ? WHERE ID_PRAC = ?";
if((isset($ID_ADRES_FROM_DATABASE)) && (!empty($ID_ADRES_FROM_DATABASE))){
    ...
    if($stmt = $mysqli->prepare($update_pracownicy)){
        $stmt->bind_param("sssssii",$name,$surname,$gender,$birthday,$pesel,$ID_ADRES_FROM_DATABASE,$ID_PRAC);
        $stmt->execute();
        $stmt->close();
    } else {
        header("Location: edit_employee.php?errorNumber=6&email=" . $_POST['old_email']);
        exit();
    }
    ...
    if($stmt = $mysqli->prepare($delete_adresy)){
        ...
        header("Location: edit_employee.php?errorNumber=7&email=" . $_POST['old_email']);
        exit();
    }
    ...
    if($stmt = $mysqli->prepare($update_pracownicy)){
        $stmt->bind_param("sssssii",$name,$surname,$gender,$birthday,$pesel,$ID_ADRES,$ID_PRAC);
        $stmt->execute();
        $stmt->close();
    } else {
        header("Location: edit_employee.php?errorNumber=6&email=" . $_POST['old_email']);
...
header("Location: database_results.php?infoNumber=3");
exit();
