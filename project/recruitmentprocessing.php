...
if(validateDirection($_POST['firstchoice'], $_POST['secondchoice'], $_POST['thirdchoice'])){
    ...
    header("Location: recruitment.php?errorNumber=" . $errorNumber);
    exit();
}
...
if($stmt = $mysqli->prepare("SELECT * FROM punkty WHERE ID_KAND = ? LIMIT 1")){
    ...
    if ($result->num_rows == 1){
        header("Location: recruitment.php?errorNumber=10");
        exit();
    }
    $stmt->close();
}
...
$insert_oceny = "INSERT INTO oceny (ID_KAND, POLSKI, OBCY, HISTORIA, MATEMATYKA, FIZYKA, BIOLOGIA, CHEMIA, WF, PASEK) VALUES (?,?,?,?,?,?,?,?,?,?)";
if($stmt = $mysqli->prepare($insert_oceny)){
    $stmt->bind_param("iiiiiiiiis",$ID_KAND,$polish,$foreign,$history,$maths,$physics,$biology,$chemistry,$pe,$distinction);
    $stmt->execute();
    $stmt->close();
} else {
    header("Location: recruitment.php?errorNumber=5");
    exit();
}

//WPROWADZENIE DANYCH DO TABELI egzaminy
$insert_egzaminy = "INSERT INTO egzaminy (ID_KAND, POLSKI, HISTORIA, MATEMATYKA, PRZYRODNICZE, OBCY_PODSTAWA) VALUES (?,?,?,?,?,?)";
if($stmt = $mysqli->prepare($insert_egzaminy)){
    $stmt->bind_param("iiiiii",$ID_KAND,$exampolish,$examhistory,$exammaths,$examscience,$examforeign);
    $stmt->execute();
    $stmt->close();
} else {
    header("Location: recruitment.php?errorNumber=6");
    exit();
}

//WPROWADZENIE DANYCH DO TABELI wybory
$insert_wybory = "INSERT INTO wybory (ID_KAND, PIERWSZY, DRUGI, TRZECI) VALUES (?,?,?,?)";
if ($stmt = $mysqli->prepare($insert_wybory)){
    $stmt->bind_param("isss",$ID_KAND,$firstchoice,$secondchoice,$thirdchoice);
    $stmt->execute();
    $stmt->close();
} else {
    header("Location: recruitment.php?errorNumber=7");
    exit();
}
...
//WPROWADZENIE DANYCH DO TABELI PUNKTY
    $insert_punkty = "INSERT INTO punkty (ID_KAND, MAT_FIZ_INF, HUMAN, SPORT, BIOL_CHEM) VALUES (?,?,?,?,?)";
    if ($stmt = $mysqli->prepare($insert_punkty)){
        $stmt->bind_param("idddd",$ID_KAND,$matfizinf_points,$human_points,$sport_points,$biolchem_points);
        $stmt->execute();
        $stmt->close();
    } else {
        header("Location: recruitment.php?errorNumber=9");
        exit();
    }

$mysqli->close();

header("Location: myaccount.php?infoNumber=2");
exit();